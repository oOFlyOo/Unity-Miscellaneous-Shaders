Shader "chenjd/SnowTrackShader"
{
    ...
    SubShader
    {
        ...
        void vert(inout appdata_full vertex)
        {
            ...
            float2 deltaPos = float2(worldPos.x - _CameraWorldPos.x, worldPos.z - _CameraWorldPos.z);
            deltaPos = deltaPos + float2(_CameraWidth / 2, _CameraHeight / 2);
            float2 deltaUV = deltaPos / float2(_CameraWidth, _CameraHeight);

            float deltaY = tex2Dlod(_SnowTrackTex, float4(deltaUV, 0, 0)).r * _SnowTrackFactor;
            deltaY = step(0, deltaUV) * step(deltaUV, 1) * deltaY;
